...
-   **out**: _&lt;array&gt;_
    ...
    -   an array containing the solution to the system `AX = B` for each square matrix. Must have the same shape as `x2` (i.e., the array corresponding to `B`). Must have a data type determined by {ref}`type-promotion` rules.
